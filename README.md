...
Production runs on **Cloudflare** and **Railway**:

| Component | Platform | Source |
|-----------|----------|--------|
| Frontend | Cloudflare Pages | [`frontend/client`](frontend/client) |
| API | Railway | [`backend`](backend) ([`Dockerfile`](backend/Dockerfile)) |
| Database | Railway PostgreSQL | Private to the Railway project |

Cloudflare DNS serves the public site; the API is exposed on a separate subdomain routed to Railway. The backend runs database migrations with Alembic before each deploy and exposes a `/health` endpoint.

Merging to `main` deploys both services automatically once CI passes.
